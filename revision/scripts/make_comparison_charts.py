...
def chart(df: pd.DataFrame, models: list, out_base: str, title: str):
    ...
    fig, axes = plt.subplots(1, len(datasets),
                             figsize=(5.4 * len(datasets), 4.6), sharey=True)
    if len(datasets) == 1:
        axes = [axes]
    width = 0.38
    for ax, ds in zip(axes, datasets):
        ...
        ax.set_xticklabels([m.replace(" (", "\n(").replace("-LoRA", "")
                            for m in present], fontsize=10, rotation=30,
                           ha="right")
        ax.set_title(DATASET_TITLES[ds], fontsize=13)
        ax.set_ylim(0, 1.05)
...
def main() -> int:
    ...
    chart(df, GROUPS["image3_corrected"],
          ...
          "(exact-match-decontaminated evaluation, corrected harness)")
